...
%     for k=1:size(r,1); % And for a given target point
        ...
        for i=1:node(n).nbrTriangle; %We select the triangle that link to this node
            ...
            r_x = r(:,1); %X coordinate of the provided point
            r_y = r(:,2); %Y coordinate of the provided point
            r_z = r(:,3); %Z coordinate of the provided point

            vni_x = basis(n).triangle(i).value(1);
            vni_y = basis(n).triangle(i).value(2);
            vni_z = basis(n).triangle(i).value(3);
            
            rpk_x = repmat( permute( basis(n).triangle(i).r_o(:,1), [2 1]), [Ntar, 1]);
            rpk_y = repmat( permute( basis(n).triangle(i).r_o(:,2), [2 1]), [Ntar, 1]);
            rpk_z = repmat( permute( basis(n).triangle(i).r_o(:,3), [2 1]), [Ntar, 1]);
            
            fCx = zeros(size(rpk_x));
            fCy = zeros(size(rpk_x));
            fCz = zeros(size(rpk_x));
        ...
        tempCx(1,:) = coef*bigSumCx;
        tempCy(1,:) = coef*bigSumCy;
        tempCz(1,:) = coef*bigSumCz;
        %C(k,n) = coef*bigSum;
%     end
    Cx(:,n) = tempCx;
    Cy(:,n) = tempCy;
