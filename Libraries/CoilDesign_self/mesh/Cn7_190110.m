function [Cx,Cy,Cz] = Cn7_190110(node,triangle,basis,r)
...
%             fCx = zeros(nbrIntegrationPoints, 1);
%             fCy = zeros(nbrIntegrationPoints, 1);
%             fCz = zeros(nbrIntegrationPoints, 1);
...
%                 fCx(l) = ck(l)*((-vni_z*(r_y-rpk_y(l))+(vni_y*(r_z-rpk_z(l))))/(normRRp^3)) * basis(n).triangle(i).air;
%                 fCy(l) = ck(l)*((-vni_x*(r_z-rpk_z(l))+(vni_z*(r_x-rpk_x(l))))/(normRRp^3)) * basis(n).triangle(i).air;
%                 fCz(l) = ck(l)*((-vni_y*(r_x-rpk_x(l))+(vni_x*(r_y-rpk_y(l))))/(normRRp^3)) * basis(n).triangle(i).air;
%                 
...
%             end
            ...
            rpk = (basis(n).triangle(i).A + basis(n).triangle(i).B)/2;
            rpk_x = rpk(1);
            rpk_y = rpk(2); 
            rpk_z = rpk(3);
            ...
            for l=1 %:nbrIntegrationPoints
                ...
                %fC = ck(l)*cross(-(r(k,1:3)-rp(l,:))/normRRp^3,vni);% To
                %optimise the executzion time, we do not use those functions
                fCx(l) = ((-vni_z*(r_y-rpk_y(l))+(vni_y*(r_z-rpk_z(l))))/(normRRp^3)) * basis(n).triangle(i).air;
                fCy(l) = ((-vni_x*(r_z-rpk_z(l))+(vni_z*(r_x-rpk_x(l))))/(normRRp^3)) * basis(n).triangle(i).air;
                fCz(l) = ((-vni_y*(r_x-rpk_x(l))+(vni_x*(r_y-rpk_y(l))))/(normRRp^3)) * basis(n).triangle(i).air;
...
%             bigSumCx = bigSumCx + sum1Cx*(2*triangle(currentTriangle_i).air)^(1/2);
...
%             bigSumCz = bigSumCz + sum1Cz*(2*triangle(currentTriangle_i).air);
            ...
            bigSumCx = bigSumCx + sum1Cx*(2*triangle(currentTriangle_i).air)^(-0/2);
            bigSumCy = bigSumCy + sum1Cy*(2*triangle(currentTriangle_i).air)^(-0/2);
            bigSumCz = bigSumCz + sum1Cz*(2*triangle(currentTriangle_i).air)^(-0/2);
            %bigSumCx = bigSumCx + scallingToCompensateTheTransform1*sum1Cx;
            

%             bigSumCy = bigSumCy + sum1Cy*(2*triangle(currentTriangle_i).air)^(1/2);

%             bigSumCy = bigSumCy + sum1Cy; %*(2*triangle(currentTriangle_i).air);
            %bigSumCy = bigSumCy + scallingToCompensateTheTransform1*sum1Cy;
            
            
%             bigSumCz = bigSumCz + sum1Cz*(2*triangle(currentTriangle_i).air)^(1/2);
%             bigSumCz = bigSumCz + sum1Cz*(2*triangle(currentTriangle_i).air);
%             bigSumCz = bigSumCz + sum1Cz; %*(2*triangle(currentTriangle_i).air);
