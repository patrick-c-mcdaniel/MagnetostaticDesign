function [ cont_xyz ] = genisocont_sphcord( ver_mesh, fac_mesh, psi_mesh, xyz_ori, tz_bas, Niso, BSFN, PLOT ) 
    ...
    addRequired( p_in, 'tz_vec');
    addRequired( p_in, 'Niso');
    
    addOptional( p_in, 'BSFN', []);
    addOptional( p_in, 'PLOT', 1);
    ...
    thtint = linspace(min(tt3(:)), max(tt3(:)), 256);
    phiint = linspace(-pi, pi, 256);
...
%     Int_rr  = scatteredInterpolant( tt3(:), pp3(:), rr3(:),'natural');
    ...
    Int_psi = scatteredInterpolant( tt3(:).*cos(pp3(:)), tt3(:).*sin(pp3(:)), psi_mesh(:),'natural');
    Int_rr  = scatteredInterpolant( tt3(:).*cos(pp3(:)), tt3(:).*sin(pp3(:)), rr3(:),'natural');
    ...
    xtp = linspace( min(thtint2d(:).*cos(phiint2d(:))), max(thtint2d(:).*cos(phiint2d(:))), 256 );
    ytp = linspace( min(thtint2d(:).*sin(phiint2d(:))), max(thtint2d(:).*sin(phiint2d(:))), 256 );
...
%     surf( thtint2d, phiint2d, psiint,'Edgecolor','none' ); colormap jet; view(2)
    ...
    psixy_smth = imgaussfilt(psixy, 2);
...
%     rrxy_smth = 
    ...
    [ cont_all ] = contour( x2tp, y2tp, psixy_smth, 30 ); colormap(cm_rb); axis equal;
