...
function [ cont_xyz ] = genisocont_sphcord_contlevels( varargin ) 
    ...
    addRequired( p_in, 'Vcont');
    
    addParameter( p_in, 'BSFN', []);
    addParameter( p_in, 'PLOT', 1);
    addParameter( p_in, 'stdSM', 2);
    addParameter( p_in, 'Icoil', 1);
    addParameter( p_in, 'Icoil_name', 'Icoil');
...
%     Niso = p_in.Results.Niso;
    Vcont = p_in.Results.Vcont;
    BSFN = p_in.Results.BSFN;
    PLOT = p_in.Results.PLOT;
    stdSM = p_in.Results.stdSM;
    Icoil = p_in.Results.Icoil;
    Icoil_name = p_in.Results.Icoil_name;
...
%     rrxy_smth = 
    ...
    figure(11); 
    [ cont_all ] = contour( x2tp, y2tp, psixy_smth, Vcont ); colormap(cm_rb); axis equal;
    ...
    writeBScont_xyz(cont_xyz, BSFN, Icoil, Icoil_name);
