...
function [ cont_xyz ] = genisocont_sphcord_v2( varargin ) 
    ...
    %                           plot; Default: 1)
    %       stdSM           - Smoothing parameter (default: 2)
...
%     Int_rr  = scatteredInterpolant( tt3(:), pp3(:), rr3(:),'natural');
    ...
    Int_psi = scatteredInterpolant( tt3(:).*cos(pp3(:)), tt3(:).*sin(pp3(:)), psi_mesh(:),'linear','none');
    Int_rr  = scatteredInterpolant( tt3(:).*cos(pp3(:)), tt3(:).*sin(pp3(:)), rr3(:),'linear','linear');
