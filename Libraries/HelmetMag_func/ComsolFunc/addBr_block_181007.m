function [ output ] = addBr_block_181007(model, domain_blk, Brxyz)

    Nshim = numel(domain_blk);
    
    for ishm =1:Nshim
        disp(['Adding block Br condition, block: ' num2str(ishm) ' / ' num2str(Nshim)]);
...
%         if ishm~=1
            ...
            model.physics('mfnc').feature(featname).selection.set(domain_blk(ishm));
%         end
        toc
