function [ hands ] = dispBmvec_200316( bmvec, ROImsk_or, plottype, plane, hands )
...
%     Bm_mat = sqrt( Bx_mat.^2 + By_mat.^2 + Bz_mat.^2 );
    ...
    switch plottype
        case 'mag'
            mosaic(permute( Bm_mat, pmat) , 6, 8, hands); colormap jet; h_cb = colorbar; caxis([min(Bm(:)) max(Bm(:))]);
            set(h_cb, 'color','white');
        case 'xyz'
            return
%             mosaic(permute( Bx_mat, pmat), 6, 8, hands(1)); colormap jet; colorbar
%             mosaic(permute( By_mat, pmat), 6, 8, hands(2)); colormap jet; colorbar
%             mosaic(permute( Bz_mat, pmat), 6, 8, hands(3)); colormap jet; colorbar
        otherwise
            mosaic(permute( Bm_mat, pmat), 6, 8, hands); colormap jet; colorbar
    end
