...
load('./optresults/cont_xyz_nose_RFLR_Example_200626.mat');
cont_xyz_all = cont_xyz_nose;
...
cont_Gy_all_dec = cell(size(cont_xyz_all));  % cell array containing all decimated contours
rn_all_dec = cell(size(cont_xyz_all));       % cell array containing surface normal vectors for all decimated contour points

len_all = 0;

TOL_end = 1e-6;

for iccc = 1:1:numel(cont_xyz_all)
    cont_Gy_1_undec = cont_xyz_all{iccc};
...
for iccc = 1:1:numel(cont_Gy_all_dec)
    ...
    %%% ver/fac for 3D mesh
    ver_tmp3d = [ c_out_tmp, c_in_tmp, c_in_tmp+TGRV*rn_tmp, c_out_tmp+TGRV*rn_tmp ];
...
FN_stl = './STL/RFLR_2D_Example_200626.stl';
...
FN_stl = './STL/RFLR_3D_Example_200626.stl';
