function [output] = proc_ROImask_190528_v01( fn_ROImask, fn_MagROIsave )
    %% Notes: 190321
    ...
    %% Notes: 190528
    %   - add non-zero initial guess magnetization for supplementary blocks
    %       (according to Halbach magnetization angular dependance)
    ...
    %%% NOTE: Works in R2017a
%     mask_pad_or_sm = imerode( imdilate( mask_pad_or, strel('sphere',4) ), strel('sphere',4) );
    
    
    %%% NOTE: Works in R2015a
    
    [xt yt zt ] = ndgrid(-4:4, -4:4, -4:4);
    Rt = 4;
    
    mskstrel = (xt.^2 + yt.^2 + zt.^2) <= Rt^2;
    
    mask_pad_or_sm = imerode( imdilate( mask_pad_or, strel(mskstrel) ), strel(mskstrel ) );
...
%     mc_op = zeros(size(xc_op));
    tc_op = pi     * ones(size(xc_op));
    pc_op = 0      * ones(size(xc_op));
    mc_op = Mcyl_e * ones(size(xc_op));
