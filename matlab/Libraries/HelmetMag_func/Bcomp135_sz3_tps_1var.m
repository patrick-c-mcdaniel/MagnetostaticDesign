function [ Bxyz ] = Bcomp135_sz3_tps_1var( D135, J3, J5, sz3tps, errcoefs )
...
%     Ys50 = Ys1.^(5:-1:0);
    ...
    %%% No implict matrix repetition in R2015a...
    Ys50 = repmat(Ys1, [1 numel(5:-1:0)]) .^ repmat( (5:-1:0), [numel(Ys1) 1]);
    ...
    M1off(1+1*(0:(Nbk-1)))   = sum(repmat(errcoefs(1,:),[Nbk, 1]) .* Ys50, 2);
    M3err(4+7*(0:(Nbk-1)))   = sum(repmat(errcoefs(2,:),[Nbk, 1]) .* Ys50, 2);
    M3err(6+7*(0:(Nbk-1)))   = sum(repmat(errcoefs(3,:),[Nbk, 1]) .* Ys50, 2);
    M5err(6+11*(0:(Nbk-1)))  = sum(repmat(errcoefs(4,:),[Nbk, 1]) .* Ys50, 2);
    M5err(8+11*(0:(Nbk-1)))  = sum(repmat(errcoefs(5,:),[Nbk, 1]) .* Ys50, 2);
    M5err(10+11*(0:(Nbk-1))) = sum(repmat(errcoefs(6,:),[Nbk, 1]) .* Ys50, 2);
    ...
    %%% analytical
    M3(4+7*(0:(Nbk-1))) = 1/4*sqrt(7/pi)   * Mblk * (1/2*Xs1.*Ys1.*Zs3 - 1/4*Xs3.*Ys1.*Zs1 - 1/4*Xs1.*Ys3.*Zs1);
    M3(6+7*(0:(Nbk-1))) = 1/4*sqrt(105/pi) * Mblk * (1/12*Xs3.*Ys1.*Zs1 - 1/12*Xs1.*Ys3.*Zs1);
    
    M5(6+11*(0:(Nbk-1)))  = 1/16*sqrt(11/pi)  *Mblk*(1/2*Xs1.*Ys1.*Zs5  - 5/6*Xs3.*Ys1.*Zs3  - 5/6*Xs1.*Ys3.*Zs3  + 3/16*Xs5.*Ys1.*Zs1 + 5/24*Xs3.*Ys3.*Zs1 + 3/16*Xs1.*Ys5.*Zs1);
    M5(8+11*(0:(Nbk-1)))  = 1/8 *sqrt(1155/pi)*Mblk*(1/24*Xs3.*Ys1.*Zs3 - 1/24*Xs1.*Ys3.*Zs3 - 1/80*Xs5.*Ys1.*Zs1 + 1/80*Xs1.*Ys5.*Zs1);
    M5(10+11*(0:(Nbk-1))) = 3/16*sqrt(385/pi) *Mblk*(1/80*Xs5.*Ys1.*Zs1 + 1/80*Xs1.*Ys5.*Zs1 - 1/24*Xs3.*Ys3.*Zs1);
