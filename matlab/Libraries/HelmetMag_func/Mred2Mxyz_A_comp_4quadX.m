function [ Mxyz, AMred, A_comp ] = Mred2Mxyz_A_comp_4quadX( Mred, ipps, ipns, inns, inps, Lhigh )
...
%         AMred = Aexp1;
    ...
    for ill = 1:numel(Lhigh)
        ...
        for imm = 1:Nm
            ...
            for iim = 1:numel(ipns) 
                A_ll( Nm*(ipns(iim)-1)+imm, (imm)+Nm*(iim-1) ) = -1*( mod(mm+ll,2)==1) + 1*(mod(mm+ll,2)==0);
                
            end
            for iim = 1:numel(inns) 
                A_ll( Nm*(inns(iim)-1)+imm, (imm)+Nm*(iim-1) ) = (-1*( mod(mm+ll,2)==1) + 1*(mod(mm+ll,2)==0)) .* ( -1*(mm<0) + 1*(mm>=0) );
