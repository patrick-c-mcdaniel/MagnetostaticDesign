function [  ] = plot_bfield_trajptsYZ( xx_gs, yy_gs, zz_gs, Bm_map_corr, Bminmax , xoff, xtol, fignum )
...
%
    ...
    for ipt = 1:numel(xx_gs)
        if abs(xx_gs(ipt) - xoff)<=xtol

            xx_vis_all(iy0) = yy_gs(ipt);
