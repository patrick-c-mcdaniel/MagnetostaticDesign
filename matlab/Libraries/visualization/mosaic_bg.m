function [ h_image ] = mosaic( imgs, row_num, col_num, fig_num, title_str, disp_range, bgcolor, bgmask )
...
if nargin < 4
    figure, imagesc(show_res,'AlphaData',1-show_bg); caxis(disp_range); axis image off, colormap gray; hold on; h_image = gca;
    ...
    figure(fig_num), imagesc(show_res,'AlphaData',1-show_bg); caxis(disp_range); axis image off, colormap gray;  h_image = gca;
    image('CData', cat(3, bgcolor(1)*ones(size(show_bg)), bgcolor(2)*ones(size(show_bg)), bgcolor(3)*ones(size(show_bg))), 'AlphaData',show_bg); hold off;
end
